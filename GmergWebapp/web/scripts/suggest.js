...
net.ContentLoader = function( component, url, method, requestParams ) {
   ...
   this.url           = url;
   this.requestParams = requestParams;inject
   this.method        = method;
}
...
TextSuggest.prototype = {
   ...
   setOptions: function(options) {
      this.options = {
         ...
         matchClassName     : 'suggestionMatch',
         matchTextWidth     : false, // true,  (changed by Mehran)
         selectionColor     : '#b6d5d1',
         matchAnywhere      : false,
         ignoreCase         : false,
         count              : 10
      }.extend(options || {});
   ...
   injectSuggestBehavior: function() {
      ...
      else		//Added by Mehran for firfox/mozilla
   ...
   callRicoAjaxEngine: function() {
      ...
      callParms.push( 'input_type='     + this.options.inputType);  //added by Mehran
   ...
   positionSuggestionsDiv: function() {
      ...
      textPos = adjustPosRelativeTomainContent({left:textPos.x, top:textPos.y});	// this is added to make it work in the new template (Mehran)
      var divStyle = this.suggestionsDiv.style;
      divStyle.top  = (textPos.top + this.textInput.offsetHeight) + "px";
...
//      divStyle.left = textPos.x + "px";
   ...
   createSuggestionsDiv: function() {
      ...
      var divStyle = this.suggestionsDiv.style;
/* this is moved to gudmaap_main.css (Mehran)      
...
TextSuggestKeyHandler.prototype = {
   ...
   keydownHandler: function(e) {
      var upArrow   = 38;
      var downArrow = 40;
      var escapeKey = 27;	//Added by Mehran
      ...
      else if ( e.keyCode == escapeKey ) {	//Added by Mehran
         if(this.textSuggest.isIE)
            this.textSuggest.textInput.blur();
         this.textSuggest.hideSuggestions();
         if(this.textSuggest.isIE)
            this.textSuggest.textInput.focus();
      } 
   },
