...
//															"GROUP_CONCAT(DISTINCT EXP_STRENGTH)",
	  ...
	  final static String query146 = "SELECT DISTINCT SUB_ACCESSION_ID,  NGS_GEO_ID, NGR_GEO_ID, SUB_SOURCE, " +
		  		"NGP_LIBRARY_STRATEGY, SUB_EMBRYO_STG, " + stageFormatConcatForNGD +", " +
		  		"SUB_SUB_DATE, NGS_SEX, NGS_DESCRIPTION, NGS_SAMPLE_NAME, " +
		  		"CASE NGS_GENOTYPE WHEN 'true' THEN 'Wild Type' ELSE CASE WHEN (SELECT DISTINCT GROUP_CONCAT(ALE_ALLELE_NAME) FROM ISH_ALLELE, LNK_SUB_ALLELE  WHERE SAL_ALE_OID_FK=ALE_OID AND SAL_SUBMISSION_FK=SUB_OID) IS NOT NULL THEN (SELECT DISTINCT GROUP_CONCAT(ALE_ALLELE_NAME) FROM ISH_ALLELE, LNK_SUB_ALLELE  WHERE SAL_ALE_OID_FK=ALE_OID AND SAL_SUBMISSION_FK=SUB_OID) ELSE (SELECT DISTINCT GROUP_CONCAT(ALE_LAB_NAME_ALLELE) FROM ISH_ALLELE, LNK_SUB_ALLELE  WHERE SAL_ALE_OID_FK=ALE_OID AND SAL_SUBMISSION_FK=SUB_OID) END  END AS GENOTYPE, " +
