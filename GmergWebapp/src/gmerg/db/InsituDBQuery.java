...
public class InsituDBQuery {
	...
	final static String name118 = "GENE_THEILER_STAGES";
	final static String query118 = "SELECT DISTINCT STG_ORDER, STG_STAGE_DISPLAY FROM ISH_SUBMISSION " +
			"JOIN ISH_PROBE ON PRB_SUBMISSION_FK = SUB_OID " +
			"JOIN REF_PROBE ON PRB_MAPROBE = RPR_OID " +
			"LEFT JOIN REF_STAGE ON SUB_STAGE_FK = STG_OID " +
			"WHERE SUB_ASSAY_TYPE IN ('ISH', 'IHC', 'TG') " +
			...
			"union " +
			"SELECT DISTINCT STG_ORDER, STG_STAGE_DISPLAY FROM REF_GENE_INFO " +
			"JOIN REF_MGI_MRK ON GNF_ID=RMM_MGIACC " +
			"JOIN MIC_BROWSE_CACHE ON RMM_SYMBOL=MBC_GNF_SYMBOL " +
			"JOIN REF_STAGE ON MBC_SUB_STAGE_FK=STG_OID " +
			"WHERE GNF_ID = ? " +
			"ORDER BY NATURAL_SORT(STG_ORDER)";
	...
	final static String name119 = "INSITU_SUBMISSION_IMAGE_ID_BY_GENE_SYMBOLID";
	final static String query119 = "SELECT CONCAT(SUB_OID, '_', IMG_FILENAME) FROM ISH_ORIGINAL_IMAGE " +
			"JOIN ISH_SUBMISSION ON IMG_SUBMISSION_FK = SUB_OID " +
			"JOIN ISH_PROBE ON PRB_SUBMISSION_FK = SUB_OID " +
			"JOIN REF_PROBE ON PRB_MAPROBE = RPR_OID " +
			"LEFT JOIN REF_STAGE ON SUB_STAGE_FK = STG_OID " +
	...
	final static String query4 = "(SELECT DISTINCT RPR_SYMBOL GENE FROM REF_PROBE " +
			...
			"UNION " +
			"(SELECT DISTINCT RSY_SYNONYM GENE, (BINARY RSY_SYNONYM)BIN FROM REF_SYNONYM " +
...
//			"JOIN REF_PROBE ON RMM_SYMBOL = RPR_SYMBOL " +
			"JOIN REF_PROBE ON RMM_MGIACC = RPR_LOCUS_TAG " +
			"JOIN ISH_PROBE ON PRB_MAPROBE = RPR_OID " +
			"JOIN ISH_SUBMISSION ON PRB_SUBMISSION_FK = SUB_OID " +
			"WHERE RMM_MGIACC IN ( ? )" +
			"AND SUB_IS_DELETED = 0 AND SUB_DB_STATUS_FK <= ? ";
...
//	final static String query116 = "SELECT DISTINCT(STG_STAGE_DISPLAY) FROM QSC_ISH_CACHE LEFT JOIN REF_STAGE ON QIC_SUB_STAGE_FK = STG_OID WHERE STG_SPECIES = ? ";
	final static String query116 = "SELECT DISTINCT(STG_STAGE_DISPLAY) FROM REF_STAGE WHERE STG_SPECIES = ? ORDER BY STG_ORDER";
	final static String name117 = "MIC_THEILER_STAGES_FROM_REF_STAGE";
//	final static String query117 = "SELECT DISTINCT(STG_STAGE_DISPLAY) FROM QSC_MIC_CACHE LEFT JOIN REF_STAGE ON QMC_SUB_STAGE_FK = STG_OID WHERE STG_SPECIES = ? ";
	final static String query117 = "SELECT DISTINCT(STG_STAGE_DISPLAY) FROM REF_STAGE WHERE STG_SPECIES = ? ORDER BY STG_ORDER";
