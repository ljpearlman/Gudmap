...
final static String NGD_ORDER_BY_LAB_AND_EXPERIMENT = " ORDER BY PER_SURNAME";
  ...
  final static String ANATOMY_PERSPECTIVE_TERMS = "SELECT DISTINCT ANO_COMPONENT_NAME AS COL1 FROM ANA_NODE, ANAD_PART_OF_PERSPECTIVE WHERE ANO_OID = POP_NODE_FK AND !POP_IS_ANCESTOR AND POP_PERSPECTIVE_FK =  'Combined Perspective (GUDMAP)' "+ 
                                                  "UNION " +
                                                  "SELECT DISTINCT SYN_SYNONYM AS COL1 FROM ANA_SYNONYM, ANA_NODE, ANAD_PART_OF_PERSPECTIVE WHERE SYN_OBJECT_FK = ANO_OID AND ANO_OID = POP_NODE_FK AND POP_PERSPECTIVE_FK = 'Combined Perspective (GUDMAP)' AND !POP_IS_ANCESTOR " +
  ...
  final static String query1 = "SELECT SUB_ACCESSION_ID, STG_STAGE_DISPLAY, SUB_ASSAY_TYPE, " +
  		...
  		"SUB_LAB_ID, SUB_ACCESSION_ID_2, SUB_OID, STG_NAME, STG_ANATOMY, STG_SPECIES FROM ISH_SUBMISSION " +
  		"LEFT JOIN REF_STAGE ON SUB_STAGE_FK = STG_OID " +
...
//query to find the number of nextgen sequences in db
  ...
  final static String ISH_NON_RENAL_SUBMISSION_QUERY_ADDITIONAL_JOIN =
	  ...
	  "JOIN ANAD_PART_OF_PERSPECTIVE ON POP_NODE_FK = ATN_NODE_FK AND POP_PERSPECTIVE_FK = 'Combined Perspective (GUDMAP)' ";
...
//  final static String query144 = ISH_BROWSE_ALL_COLUMNS + ISH_BROWSE_ALL_TABLES
  ...
  final static String query217 = "SELECT DISTINCT SUBSTR(ATN_PUBLIC_ID, 6), CONCAT(ANO_COMPONENT_NAME, '(', ATN_PUBLIC_ID, ')') COMPONENT FROM ANA_TIMED_NODE " +
  		...
  		"JOIN ANAD_PART_OF_PERSPECTIVE ON POP_NODE_FK = ANO_OID AND POP_PERSPECTIVE_FK = 'Combined Perspective (GUDMAP)' " +
  		"WHERE STG_NAME = ? " +
...
//  		"WHERE GNF_SYMBOL = ? " +
  ...
  final static String query230 = "SELECT AME_M_HEADER_FK, AME_PROBE_SET_FK, AME_VALUE_RMA, AMS_MEDIAN, AMS_STD FROM MIC_ANAL_MSTR_EXPRESSION " +
	...
	final static String query285 = "SELECT EXP_COMPONENT_ID,DEN_RELATIVE_TO_AGE,DEN_RELATIVE_TO_TOTAL,DEN_DIRECTION_CHANGE,DEN_MAGNITUDE_CHANGE FROM ISH_EXPRESSION,ISH_DENSITY WHERE EXP_OID = DEN_EXPRESSION_FK AND (CONCAT('GUDMAP:',EXP_SUBMISSION_FK) = ?)";
  ...
  static String submissionHistoryQuery = "SELECT SBH_ACCESSION_ID, " +  // 0
    ...
    public static String getComponentRelations(int numEnteredComps, String col1, String col2, String col3) {
...
//    			"AND "+col2+" IN (");
   			    " FROM ANA_NODE " +
    			"JOIN ANAD_RELATIONSHIP_TRANSITIVE ON "+col3+" = ANO_OID " +
...
//    	String compsFromSynsAndIdsQ = "SELECT DISTINCT ATN_PUBLIC_ID FROM ANA_NODE, ANA_TIMED_NODE WHERE ANO_OID = ATN_NODE_FK AND ANO_COMPONENT_NAME RLIKE ?" +
//    			" UNION SELECT DISTINCT ATN_PUBLIC_ID FROM ANA_NODE, ANA_TIMED_NODE, ANA_SYNONYM WHERE SYN_OBJECT_FK = ANO_OID AND ANO_OID = ATN_NODE_FK AND SYN_SYNONYM RLIKE ?" +
...
//    			" UNION SELECT DISTINCT ATN_PUBLIC_ID FROM ANA_NODE, ANA_TIMED_NODE WHERE ATN_NODE_FK = ANO_OID AND ANO_PUBLIC_ID = ?";
    	...
    	String compsFromSynsAndIdsQ = "SELECT DISTINCT ANO_PUBLIC_ID FROM ANA_NODE WHERE ANO_COMPONENT_NAME RLIKE ?" +
    			" UNION SELECT DISTINCT ANO_PUBLIC_ID FROM ANA_NODE, ANA_SYNONYM WHERE SYN_OBJECT_FK = ANO_OID AND SYN_SYNONYM RLIKE ?" +
    			" UNION SELECT DISTINCT ANO_PUBLIC_ID FROM ANA_NODE, ANA_TIMED_NODE WHERE ATN_NODE_FK = ANO_OID AND ATN_PUBLIC_ID = ?" +
    			" UNION SELECT DISTINCT ANO_PUBLIC_ID FROM ANA_NODE, ANA_TIMED_NODE WHERE ATN_NODE_FK = ANO_OID AND ANO_PUBLIC_ID = ?";
